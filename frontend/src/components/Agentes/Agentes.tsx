     ...
     <div className="flex items-center justify-between gap-3 sm:flex-nowrap">
      <div className="flex items-center gap-3 sm:gap-5">
        ...
        <div className="w-full">
          <p className="font-semibold text-primary">Gerenciamento de agentes</p>
          <p className="text-sm font-medium">Adicionar, visualizar e excluir agentes</p>
        </div>
      </div>
            ...
            <tbody>
              {agentes.slice(0, 4).map((agente) => (
                <tr key={agente.id}>
                  ...
                  <td className="p-2">{agente.email}</td>
                  <td className="p-2 text-sm">{agente.created.toDateString().slice(4, 16)}</td>
                  <td className="p-2">{agente.isAdmin ? "Admin" : "Agente"}</td>
                  <td className="p-2">{agente.isActive ? "ativo" : "inativo"}</td>
                  <td className="p-2">
                    <div className="flex gap-2">
                      <Link href={`/dashboard/users/${agente.id}`}>
